...
struct CtxInner {
    ...
    // (removed) replay-only correlation counter; replay engine now owns its ids
...
impl CtxInner {
    fn new(history: Vec<Event>, execution_id: u64) -> Self {
        ...
        let _hist_len_u64 = history.len() as u64; // reserved for future use
    ...
    // (removed) replay_next_id; decisions generated in replay engine
}
