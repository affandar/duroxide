            ...
            match &event.kind {
                ...
                EventKind::ActivityScheduled { name, input: inp } => {
                    if let Some(result) = self.match_and_bind_schedule(
                        ...
                        &mut schedule_kinds,
                        event,
                        ActionKind::Activity { name: name.clone(), input: inp.clone() },
                    ) {
                        return result;
                    }
                }
                ...
                EventKind::TimerCreated { fire_at_ms } => {
                    if let Some(result) = self.match_and_bind_schedule(
                        ...
                        &mut schedule_kinds,
                        event,
                        ActionKind::Timer { fire_at_ms: *fire_at_ms },
                    ) {
                        return result;
                    }
                }
                ...
                EventKind::ExternalSubscribed { name } => {
                    ...
                    ctx.inner.lock().expect("Mutex should not be poisoned")
                ...
                EventKind::SubOrchestrationScheduled { name, instance, input: inp, .. } => {
                    if let Some(result) = self.match_and_bind_schedule(
                        ...
                        &mut schedule_kinds,
                        event,
                        ActionKind::SubOrch { name: name.clone(), instance: instance.clone(), input: inp.clone() },
                    ) {
                        return result;
                    }
                }
...
fn action_to_event(action: &Action, instance: &str, execution_id: u64, event_id: u64) -> Option<Event> {
    let kind = match action {
        ...
        Action::CreateTimer { fire_at_ms, .. } => {
            EventKind::TimerCreated { fire_at_ms: *fire_at_ms }
        }
        Action::WaitExternal { name, .. } => {
            EventKind::ExternalSubscribed { name: name.clone() }
        }
        Action::StartSubOrchestration { name, instance: sub_instance, input, .. } => {
...
fn action_matches_event_kind(action: &Action, event_kind: &EventKind) -> bool {
    match (action, event_kind) {
        (
            Action::CallActivity { name, input, .. },
            EventKind::ActivityScheduled { name: en, input: ei },
        ) => name == en && input == ei,
        
        (
            Action::CreateTimer { fire_at_ms, .. },
            EventKind::TimerCreated { fire_at_ms: ef },
        ) => {
        ...
        (
            ...
            EventKind::SubOrchestrationScheduled { name: en, input: ei, .. },
        ) => name == en && input == ei,
        
        (
            Action::SystemCall { op, .. },
            EventKind::SystemCall { op: eop, .. },
        ) => op == eop,
