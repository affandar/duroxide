...
impl ReplayEngine {
    ...
    pub fn prep_completions(&mut self, messages: Vec<WorkItem>) {
        ...
        for msg in messages {
            ...
            let already_in_delta = match &msg {
                ...
                WorkItem::TimerFired { id, .. } => self.history_delta.iter().any(|e| {
                    e.source_event_id == Some(*id) && matches!(&e.kind, EventKind::TimerFired { .. })
                }),
                ...
                WorkItem::ExternalRaised { name, data, .. } => self.history_delta.iter().any(|e| {
                    matches!(&e.kind, EventKind::ExternalEvent { name: n, data: d } if n == name && d == data)
                }),
            ...
            let event_opt = match msg {
                ...
                WorkItem::ExternalRaised { name, data, .. } => {
                    ...
                    let subscribed = self
                        .baseline_history
                        .iter()
                        .any(|e| matches!(&e.kind, EventKind::ExternalSubscribed { name: hist_name } if hist_name == &name));
                    if subscribed {
                        Some(Event::new(
                            &self.instance,
                            self.execution_id,
    ...
    fn is_completion_already_in_history(&self, msg: &WorkItem) -> bool {
        match msg {
            WorkItem::ActivityCompleted { id, .. } => self.baseline_history.iter().any(|e| {
                e.source_event_id == Some(*id) && matches!(&e.kind, EventKind::ActivityCompleted { .. })
            }),
            WorkItem::TimerFired { id, .. } => self.baseline_history.iter().any(|e| {
                e.source_event_id == Some(*id) && matches!(&e.kind, EventKind::TimerFired { .. })
            }),
            WorkItem::SubOrchCompleted { parent_id, .. } => self.baseline_history.iter().any(|e| {
                e.source_event_id == Some(*parent_id)
                    && matches!(&e.kind, EventKind::SubOrchestrationCompleted { .. })
            }),
            WorkItem::SubOrchFailed { parent_id, .. } => self.baseline_history.iter().any(|e| {
                e.source_event_id == Some(*parent_id)
                    && matches!(&e.kind, EventKind::SubOrchestrationFailed { .. })
            }),
