...
async fn async_block_join_many() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ...
        for (i, r) in results.into_iter().enumerate() {
            match r {
                Ok(s) => outputs.push(s),
                Err(e) => outputs.push(format!("block{}:error:{e}", i)),
            }
        }
    ...
    let client = Client::new(store.clone());
    client
        .start_orchestration("join-many-1", "JoinMany", "")
        .await
        .unwrap();
...
async fn async_block_sequential() {
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                output.starts_with("final:"),
                "Should have final prefix: {output}"
            );
            assert!(
...
async fn async_block_vs_durable_future() {
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                output.starts_with("winner:0,"),
                "Single future should win: {output}"
            );
            assert!(
...
async fn async_block_select3_with_timers() {
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                output.starts_with("winner:0,"),
                "Block A should win: {output}"
            );
            assert!(
                output.contains("A:work:A"),
                "Block A result incorrect: {output}"
            );
        }
        runtime::OrchestrationStatus::Failed { details } => {
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                ...
                "Result should contain step:1: {output}"
            );
            assert!(
                output.contains("step:2"),
                "Result should contain step:2: {output}"
            );
            assert!(
                output.contains("step:3"),
                "Result should contain step:3: {output}"
            );
        }
        runtime::OrchestrationStatus::Failed { details } => {
            panic!("orchestration failed: {}", details.display_message())
...
async fn async_block_suborchestration_wins_race() {
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                output.starts_with("winner:0,"),
                "Sub-orch block should win: {output}"
            );
            assert!(
...
async fn async_block_suborchestration_loses_race() {
    ...
    if let Some(child_id) = child_instance {
        ...
        match child_status {
            runtime::OrchestrationStatus::Completed { output } => {
                panic!(
                    "Child should NOT have completed (it lost the race), but got: {output}"
                );
            }
...
async fn async_block_multiple_suborchestrations_joined() {
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                output.contains("childA:"),
                "Should have childA result: {output}"
            );
            assert!(output.contains("block2:"), "Should have block2: {output}");
            assert!(
                output.contains("childB:"),
                "Should have childB result: {output}"
            );
            assert!(output.contains("block3:"), "Should have block3: {output}");
            assert!(
                output.contains("childC:"),
                "Should have childC result: {output}"
            );
        }
        runtime::OrchestrationStatus::Failed { details } => {
            panic!("orchestration failed: {}", details.display_message())
...
async fn async_block_suborchestration_racing_timeout() {
    ...
    if let Some(instance_id) = slow_child_instance {
        ...
        match status {
            runtime::OrchestrationStatus::Completed { output } => {
                panic!(
                    "SlowChild should NOT have completed (it lost the race), got: {output}"
                );
            }
            runtime::OrchestrationStatus::Failed { .. }
            | runtime::OrchestrationStatus::Running
        ...
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output } => {
            ...
            assert!(
                ...
                "Should have child results: {output}"
            );
        }
        runtime::OrchestrationStatus::Failed { details } => {
            panic!("orchestration failed: {}", details.display_message())
