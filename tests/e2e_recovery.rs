...
async fn recovery_across_restart_core<F1, F2>(
    make_store_stage1: F1,
    make_store_stage2: F2,
    instance: String,
) where
    ...
    F2: Fn() -> StdArc<dyn HistoryStore>,
{
    let orchestrator = |ctx: OrchestrationContext| async move {
        let s1 = ctx
            .schedule_activity("Step", "1")
            .into_activity()
            .await
            .unwrap();
        let s2 = ctx
            .schedule_activity("Step", "2")
            .into_activity()
            .await
            .unwrap();
        let _ = ctx.schedule_wait("Resume").into_event().await;
        let s3 = ctx
            .schedule_activity("Step", "3")
            .into_activity()
            .await
            .unwrap();
        let s4 = ctx
            .schedule_activity("Step", "4")
            .into_activity()
            .await
            .unwrap();
        format!("{s1}{s2}{s3}{s4}")
    ...
    let handle1 = rt1
        .clone()
        .spawn_instance_to_completion(&instance, orchestrator)
        .await;
...
async fn recovery_across_restart_fs_provider() {
    ...
    let count = |inp: &str| {
        hist.iter().filter(|e| matches!(e, Event::ActivityScheduled { name, input, .. } if name == "Step" && input == inp)).count()
    };
    assert_eq!(count("1"), 1);
    assert_eq!(count("2"), 1);
    assert_eq!(count("3"), 1);
