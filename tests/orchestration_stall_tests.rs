...
fn select2_loser_timer_does_not_block_later_timer() {
    ...
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted {
        ...
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled {
            name: "FastActivity".to_string(),
            input: "input1".to_string(),
        }),
        // First select2: timeout timer created (event 3)
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::TimerCreated {
            fire_at_ms: 15000,
        }),
        // First activity completes (event 4) - WINS the race
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted {
            result: "result1".to_string(),
        }),
        // Second select2: activity scheduled (event 5)
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled {
            name: "FastActivity".to_string(),
            input: "input2".to_string(),
        }),
        // Second select2: timeout timer created (event 6)
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::TimerCreated {
            fire_at_ms: 30000,
        }),
        // Second activity completes (event 7) - WINS the race
        Event::with_event_id(7, inst.clone(), 1, Some(5), EventKind::ActivityCompleted {
            result: "result2".to_string(),
        }),
        // Third select2: sleep timer created (event 8)
        Event::with_event_id(8, inst.clone(), 1, None, EventKind::TimerCreated {
            fire_at_ms: 30000,
        }),
        // External wait subscription (event 9)
        Event::with_event_id(9, inst.clone(), 1, None, EventKind::ExternalSubscribed {
            name: "InstanceDeleted".to_string(),
        }),
        // First stale timer fires (event 10) - from first select2, LOSER
        Event::with_event_id(10, inst.clone(), 1, Some(3), EventKind::TimerFired {
            fire_at_ms: 15000,
        }),
        // Second stale timer fires (event 11) - from second select2, LOSER
        Event::with_event_id(11, inst.clone(), 1, Some(6), EventKind::TimerFired {
            fire_at_ms: 30000,
        }),
        // Third timer fires (event 12) - from third select2, should be consumed
        Event::with_event_id(12, inst.clone(), 1, Some(8), EventKind::TimerFired {
            fire_at_ms: 30000,
        }),
    ];
...
fn single_stale_loser_handled() {
    ...
    let inst = "test-inst".to_string();
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted { name: "Test".to_string(), version: "1.0.0".to_string(), input: "".to_string(), parent_instance: None, parent_id: None }),
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Fast".to_string(), input: "input".to_string() }),
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 10000 }),
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "done".to_string() }),
...
fn select2_loser_activity_does_not_block_later_timer() {
    ...
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted { name: "Test".to_string(), version: "1.0.0".to_string(), input: "".to_string(), parent_instance: None, parent_id: None }),
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "SlowActivity".to_string(), input: "input".to_string() }),
        ...
        Event::with_event_id(6, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "slow_result".to_string() }),
...
fn multiple_retry_pattern_then_timer_completes() {
    ...
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted { name: "Test".to_string(), version: "1.0.0".to_string(), input: "".to_string(), parent_instance: None, parent_id: None }),
        // Retry 1
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input0".to_string() }),
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // Retry 2
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input1".to_string() }),
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(7, inst.clone(), 1, Some(5), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // Retry 3
        Event::with_event_id(8, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input2".to_string() }),
        Event::with_event_id(9, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(10, inst.clone(), 1, Some(8), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // Final timer
        Event::with_event_id(11, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 10000 }),
        // Stale timers from all 3 retries
        Event::with_event_id(12, inst.clone(), 1, Some(3), EventKind::TimerFired { fire_at_ms: 30000 }),
        Event::with_event_id(13, inst.clone(), 1, Some(6), EventKind::TimerFired { fire_at_ms: 30000 }),
        Event::with_event_id(14, inst.clone(), 1, Some(9), EventKind::TimerFired { fire_at_ms: 30000 }),
        // Final timer fires
        Event::with_event_id(15, inst.clone(), 1, Some(11), EventKind::TimerFired { fire_at_ms: 10000 }),
    ];
...
fn instance_actor_pattern_completes() {
    ...
    let inst = "test-inst".to_string();
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted { name: "InstanceActor".to_string(), version: "1.0.0".to_string(), input: "{}".to_string(), parent_instance: None, parent_id: None }),
        // First retry
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "GetConnection".to_string(), input: "input".to_string() }),
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 15000 }),
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "conn".to_string() }),
        // Second retry
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "TestConnection".to_string(), input: "input".to_string() }),
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(7, inst.clone(), 1, Some(5), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // Sleep/signal select2
        Event::with_event_id(8, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(9, inst.clone(), 1, None, EventKind::ExternalSubscribed { name: "InstanceDeleted".to_string() }),
        // Stale timers from retries
        Event::with_event_id(10, inst.clone(), 1, Some(3), EventKind::TimerFired { fire_at_ms: 15000 }),
        Event::with_event_id(11, inst.clone(), 1, Some(6), EventKind::TimerFired { fire_at_ms: 30000 }),
        // Sleep timer fires
        Event::with_event_id(12, inst.clone(), 1, Some(8), EventKind::TimerFired { fire_at_ms: 30000 }),
    ];
...
fn new_execution_starts_clean() {
    ...
    let inst = "test-inst".to_string();
    let history = vec![
        Event::with_event_id(1, inst.clone(), 2, None, EventKind::OrchestrationStarted { name: "Test".to_string(), version: "1.0.0".to_string(), input: "continued".to_string(), parent_instance: None, parent_id: None }),
        Event::with_event_id(2, inst.clone(), 2, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input".to_string() }),
        Event::with_event_id(3, inst.clone(), 2, None, EventKind::TimerCreated { fire_at_ms: 10000 }),
        Event::with_event_id(4, inst.clone(), 2, Some(2), EventKind::ActivityCompleted { result: "done".to_string() }),
    ];
...
fn stale_completion_same_execution_handled() {
    ...
    let history = vec![
        ...
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Fast".to_string(), input: "input".to_string() }),
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "fast_done".to_string() }),
        // Second activity
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "MoreWork".to_string(), input: "data".to_string() }),
        ...
        Event::with_event_id(7, inst.clone(), 1, Some(5), EventKind::ActivityCompleted { result: "more_done".to_string() }),
    ];
...
fn sequential_select2s_loser_from_first_does_not_block_second() {
    ...
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted { name: "Test".to_string(), version: "1.0.0".to_string(), input: "".to_string(), parent_instance: None, parent_id: None }),
        // First select2
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Fast1".to_string(), input: "input".to_string() }),
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 100000 }),
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "done1".to_string() }),
        // Second select2
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Slow2".to_string(), input: "input".to_string() }),
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 1000 }),
        // Stale timer from first select2
        Event::with_event_id(7, inst.clone(), 1, Some(3), EventKind::TimerFired { fire_at_ms: 100000 }),
        // Second timer wins
        Event::with_event_id(8, inst.clone(), 1, Some(6), EventKind::TimerFired { fire_at_ms: 1000 }),
        // Stale activity from second select2
        Event::with_event_id(9, inst.clone(), 1, Some(5), EventKind::ActivityCompleted { result: "done2".to_string() }),
    ];
...
fn join_then_select2_completes() {
    ...
    let history = vec![
        Event::with_event_id(1, inst.clone(), 1, None, EventKind::OrchestrationStarted { name: "Test".to_string(), version: "1.0.0".to_string(), input: "".to_string(), parent_instance: None, parent_id: None }),
        // Join activities
        Event::with_event_id(2, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task1".to_string(), input: "input1".to_string() }),
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task2".to_string(), input: "input2".to_string() }),
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "r1".to_string() }),
        Event::with_event_id(5, inst.clone(), 1, Some(3), EventKind::ActivityCompleted { result: "r2".to_string() }),
        // Select2
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task3".to_string(), input: "input3".to_string() }),
        Event::with_event_id(7, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(8, inst.clone(), 1, Some(6), EventKind::ActivityCompleted { result: "r3".to_string() }),
        // Stale timeout
...
fn select2_then_join_completes() {
    ...
    let history = vec![
        ...
        Event::with_event_id(4, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "done".to_string() }),
        // Join activities
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task2".to_string(), input: "input2".to_string() }),
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task3".to_string(), input: "input3".to_string() }),
        ...
        // Join completions
        Event::with_event_id(8, inst.clone(), 1, Some(5), EventKind::ActivityCompleted { result: "r2".to_string() }),
        Event::with_event_id(9, inst.clone(), 1, Some(6), EventKind::ActivityCompleted { result: "r3".to_string() }),
    ];
...
fn mixed_loser_types_do_not_block() {
    ...
    let history = vec![
        ...
        Event::with_event_id(5, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "FastActivity".to_string(), input: "input".to_string() }),
        Event::with_event_id(6, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 100000 }),
        Event::with_event_id(7, inst.clone(), 1, Some(5), EventKind::ActivityCompleted { result: "fast".to_string() }),
        // Final timer
        Event::with_event_id(8, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 5000 }),
        // Stale activity (from first select2 loser)
        Event::with_event_id(9, inst.clone(), 1, Some(2), EventKind::ActivityCompleted { result: "slow".to_string() }),
        // Stale timer (from second select2 loser)
        Event::with_event_id(10, inst.clone(), 1, Some(6), EventKind::TimerFired { fire_at_ms: 100000 }),
        // Final timer fires
...
fn sequential_select2s_accumulate_cancelled_sources() {
    ...
    let history = vec![
        ...
        Event::with_event_id(10, inst.clone(), 1, Some(8), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // select2 #3
        Event::with_event_id(11, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input3".to_string() }),
        Event::with_event_id(12, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(13, inst.clone(), 1, Some(11), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // select2 #4
        Event::with_event_id(14, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input4".to_string() }),
        Event::with_event_id(15, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 30000 }),
        Event::with_event_id(16, inst.clone(), 1, Some(14), EventKind::ActivityCompleted { result: "ok".to_string() }),
        // Final timer
        Event::with_event_id(17, inst.clone(), 1, None, EventKind::TimerCreated { fire_at_ms: 1000 }),
        // All 5 stale timers fire
        Event::with_event_id(18, inst.clone(), 1, Some(3), EventKind::TimerFired { fire_at_ms: 30000 }),
        Event::with_event_id(19, inst.clone(), 1, Some(6), EventKind::TimerFired { fire_at_ms: 30000 }),
        Event::with_event_id(20, inst.clone(), 1, Some(9), EventKind::TimerFired { fire_at_ms: 30000 }),
        Event::with_event_id(21, inst.clone(), 1, Some(12), EventKind::TimerFired { fire_at_ms: 30000 }),
        Event::with_event_id(22, inst.clone(), 1, Some(15), EventKind::TimerFired { fire_at_ms: 30000 }),
        // Final timer fires
        Event::with_event_id(23, inst.clone(), 1, Some(17), EventKind::TimerFired { fire_at_ms: 1000 }),
    ];
...
fn schedule_order_mismatch_triggers_nondeterminism() {
    ...
    let mismatched_history = vec![
        ...
        Event::with_event_id(3, inst.clone(), 1, None, EventKind::ActivityScheduled { name: "Task".to_string(), input: "input".to_string() }),
    ];
