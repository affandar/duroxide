...
async fn external_duplicate_workitems_dedup_fs() {
    ...
    let orchestration_registry = OrchestrationRegistry::builder()
        .register("WaitEvt", orch)
        .build();
    let activity_registry = ActivityRegistry::builder().build();
    let rt = runtime::Runtime::start_with_store(store.clone(), StdArc::new(activity_registry), orchestration_registry).await;
    ...
    let external_events: Vec<&Event> = hist.iter().filter(|e| matches!(e, Event::ExternalEvent { name, .. } if name == "Evt")).collect();
    assert_eq!(external_events.len(), 1, "expected 1 ExternalEvent, got {}", external_events.len());
...
async fn timer_duplicate_workitems_dedup_fs() {
    ...
    assert!(common::wait_for_history(store.clone(), inst, |h| h.iter().any(|e| matches!(e, Event::TimerCreated { .. })), 2_000).await);
    let (id, fire_at_ms) = {
        let hist = store.read(inst).await;
        let mut t_id = 0u64; let mut t_fire = 0u64;
        for e in hist.iter() { if let Event::TimerCreated { id, fire_at_ms } = e { t_id = *id; t_fire = *fire_at_ms; break; } }
        (t_id, t_fire)
    };
...
async fn activity_duplicate_completion_workitems_dedup_fs() {
    ...
    let activity_registry = ActivityRegistry::builder()
        ...
        .build();
    let orch = |ctx: OrchestrationContext, _input: String| async move {
        let out = ctx.schedule_activity("SlowEcho", "x".to_string()).into_activity().await.unwrap();
        Ok(out)
    };
    let orchestration_registry = OrchestrationRegistry::builder()
        .register("OneSlowAct", orch)
        .build();
    let rt = runtime::Runtime::start_with_store(store.clone(), StdArc::new(activity_registry), orchestration_registry).await;
    ...
    assert!(common::wait_for_history(store.clone(), inst, |h| h.iter().any(|e| matches!(e, Event::ActivityScheduled { name, .. } if name == "SlowEcho")), 2_000).await);
    let id = {
        let hist = store.read(inst).await;
        let mut t_id = 0u64; for e in hist.iter() { if let Event::ActivityScheduled { id, name, .. } = e { if name == "SlowEcho" { t_id = *id; break; } } }
        t_id
    };
    ...
    let hist = store.read(inst).await;
    let acts: Vec<&Event> = hist.iter().filter(|e| matches!(e, Event::ActivityCompleted { id: cid, .. } if *cid == id)).collect();
    assert_eq!(acts.len(), 1, "expected 1 ActivityCompleted for id={}, got {}", id, acts.len());
