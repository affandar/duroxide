...
async fn sample_hello_world_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, input: String| async move {
        ...
        let res = ctx.schedule_activity_v2("Hello", "Rust").await?;
        ctx.trace_info(format!("hello_world result={res} "));
        let res1 = ctx.schedule_activity_v2("Hello", input).await?;
...
async fn sample_basic_control_flow_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        let flag = ctx.schedule_activity_v2("GetFlag", "").await.unwrap();
        ctx.trace_info(format!("control_flow flag decided = {flag}"));
        if flag == "yes" {
            Ok(ctx.schedule_activity_v2("SayYes", "").await.unwrap())
        } else {
            Ok(ctx.schedule_activity_v2("SayNo", "").await.unwrap())
        }
    };
...
async fn sample_loop_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ...
        for i in 0..3 {
            acc = ctx.schedule_activity_v2("Append", acc).await.unwrap();
...
async fn sample_error_handling_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        match ctx.schedule_activity_v2("Fragile", "bad").await {
            Ok(v) => {
            ...
            Err(e) => {
                ...
                let rec = ctx.schedule_activity_v2("Recover", "").await.unwrap();
                if rec != "recovered" {
...
async fn sample_timeout_with_timer_race_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        let mut act = std::pin::pin!(ctx.schedule_activity_v2("LongOp", ""));
        let mut timer = std::pin::pin!(ctx.schedule_timer_v2(Duration::from_millis(100)));
        futures::select! {
            result = act => match result {
                Ok(s) => Ok(s),
                Err(e) => Err(e),
            },
            _ = timer => Err("timeout".to_string()),
        }
    };
...
async fn sample_select2_activity_vs_external_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        let mut act = std::pin::pin!(ctx.schedule_activity_v2("Sleep", ""));
        let mut evt = std::pin::pin!(ctx.schedule_wait_v2("Go"));
        futures::select! {
            result = act => match result {
                Ok(s) => Ok(format!("activity:{s}")),
                Err(e) => Err(e),
            },
            payload = evt => Ok(format!("event:{payload}")),
        }
    };
...
async fn dtf_legacy_gabbar_greetings_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        // Schedule two greetings in parallel using futures::join!
        let (a, b) = futures::join!(
            ctx.schedule_activity_v2("Greetings", "Gabbar"),
            ctx.schedule_activity_v2("Greetings", "Samba")
        );
        let mut vals: Vec<String> = vec![a.unwrap(), b.unwrap()];
...
async fn sample_status_polling_fs() {
    ...
    let orchestration = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        ctx.schedule_timer_v2(Duration::from_millis(20)).await;
        Ok("done".to_string())
    };
    let orchestration_registry = OrchestrationRegistry::builder()
...
async fn sample_sub_orchestration_basic_fs() {
    ...
    let child_upper = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        let up = ctx.schedule_activity_v2("Upper", input).await.unwrap();
        Ok(up)
    };
    let parent = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        let r = ctx
            .schedule_sub_orchestration_v2("ChildUpper", input)
            .await
            .unwrap();
        Ok(format!("parent:{r}"))
    };
...
async fn sample_sub_orchestration_fanout_fs() {
    ...
    let child_sum = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        let s = ctx.schedule_activity_v2("Add", input).await.unwrap();
        Ok(s)
    };
    let parent = |ctx: OrchestrationContext, _input: String| async move {
        ctx.initialize_v2();
        let (a, b) = futures::join!(
            ctx.schedule_sub_orchestration_v2("ChildSum", "1,2"),
            ctx.schedule_sub_orchestration_v2("ChildSum", "3,4")
        );
        let nums: Vec<i64> = vec![
            a.unwrap().parse::<i64>().unwrap(),
            b.unwrap().parse::<i64>().unwrap(),
        ];
...
async fn sample_sub_orchestration_chained_fs() {
    ...
    let leaf = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        Ok(ctx.schedule_activity_v2("AppendX", input).await.unwrap())
    };
    let mid = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        let r = ctx
            .schedule_sub_orchestration_v2("Leaf", input)
            .await
            .unwrap();
        Ok(format!("{r}-mid"))
    };
    let root = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        let r = ctx
            .schedule_sub_orchestration_v2("Mid", input)
            .await
            .unwrap();
        Ok(format!("root:{r}"))
    };
...
async fn sample_detached_orchestration_scheduling_fs() {
    ...
    let chained = |ctx: OrchestrationContext, input: String| async move {
        ctx.initialize_v2();
        ctx.schedule_timer_v2(Duration::from_millis(5)).await;
        Ok(ctx.schedule_activity_v2("Echo", input).await.unwrap())
    };
