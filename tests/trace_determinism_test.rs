use duroxide::{
    ...
    providers::Provider,
    runtime::{self, OrchestrationStatus, registry::ActivityRegistry},
};
use std::sync::Arc;
...
async fn test_trace_deterministic_in_history() {
    ...
    assert_eq!(
        trace_events.len(),
        3,
        "Should have exactly three trace events in history"
    );
    ...
    let rt2 = runtime::Runtime::start_with_store(
        ...
        OrchestrationRegistry::builder().register("test_orch", orch).build(),
    )
    .await;
    ...
    let trace_events_after: Vec<_> = history_after_replay
        .iter()
        .filter(|e| matches!(e, duroxide::Event::SystemCall { op, .. } if op.starts_with("trace:")))
        .collect();

    assert_eq!(
        trace_events_after.len(),
        3,
        "No new trace events should be added during replay"
    );
...
async fn test_trace_fire_and_forget() {
    ...
    // Wait for completion
    let status = client
        .wait_for_orchestration("instance-3", std::time::Duration::from_millis(1000))
        .await
        .unwrap();
