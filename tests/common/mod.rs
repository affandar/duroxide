...
where
    F: Fn(&Vec<Event>) -> bool,
{
    wait_for_history_event(store, instance, |hist| if predicate(hist) { Some(()) } else { None }, timeout_ms)
        .await
        .is_some()
}
...
pub async fn wait_for_history_event<T, F>(store: StdArc<dyn HistoryStore>, instance: &str, selector: F, timeout_ms: u64) -> Option<T>
where
    T: Clone,
    F: Fn(&Vec<Event>) -> Option<T>,
{
    ...
    loop {
        ...
        if let Some(e) = selector(&hist) { return Some(e); }
