...
async fn dispatcher_enqueues_timer_schedule_then_completes_fs() {
    ...
    let ok = wait_for_history(store_dyn.clone(), inst, |h| h.iter().any(|e| matches!(e, Event::OrchestrationCompleted { output } if output == "ok")), 5_000).await;
...
async fn dispatcher_enqueues_start_orchestration_to_orch_queue_fs() {
    ...
    let reg = OrchestrationRegistry::builder().register("Child", child).register("Parent", parent).build();
    let rt = runtime::Runtime::start_with_store(store_dyn.clone(), StdArc::new(acts), reg).await;

    let _h = rt.clone().start_orchestration("inst-parent", "Parent", "").await.unwrap();

    // Child should complete with input "A".
    let ok = wait_for_history(store_dyn.clone(), "W1", |h| h.iter().any(|e| matches!(e, Event::OrchestrationCompleted { output } if output == "A")), 5_000).await;
