...
fn activity_scheduled_no_completion() {
    ...
    assert!(engine.pending_actions().is_empty(), "No new actions expected (already scheduled)");
}
...
fn external_subscribed_no_event() {
    ...
    assert_continue(&result);
    assert!(engine.pending_actions().is_empty(), "No new actions expected (already subscribed)");
}
