...
use duroxide::{Event, EventKind, OrchestrationContext, OrchestrationHandler, ErrorDetails, ConfigErrorKind};
...
impl OrchestrationHandler for MultiScheduleNoAwaitHandler {
    async fn invoke(&self, ctx: OrchestrationContext, _input: String) -> Result<String, String> {
        // Schedule activities but don't await them
        for (name, input) in &self.activities {
            let _ = ctx.schedule_activity(name, input);
        }
        Ok("done".to_string())
    }
}
...
impl OrchestrationHandler for JoinActivitiesHandler {
    async fn invoke(&self, ctx: OrchestrationContext, _input: String) -> Result<String, String> {
        let futures: Vec<_> = self.activities.iter()
...
impl PanicHandler {
    pub fn new(msg: &str) -> Arc<Self> {
        Arc::new(Self { message: msg.to_string() })
    }
}
...
pub fn assert_continue(result: &TurnResult) {
    assert!(
        ...
        "Expected TurnResult::Continue, got {:?}",
        result
    );
}
...
pub fn assert_failed(result: &TurnResult) {
    assert!(
        ...
        "Expected TurnResult::Failed, got {:?}",
        result
    );
}
...
pub fn assert_nondeterminism(result: &TurnResult) {
    match result {
        TurnResult::Failed(ErrorDetails::Configuration { kind: ConfigErrorKind::Nondeterminism, .. }) => {}
...
pub fn assert_cancelled(result: &TurnResult) {
    assert!(
        ...
        "Expected TurnResult::Cancelled, got {:?}",
        result
    );
}
...
pub fn has_activity_action(engine: &ReplayEngine, name: &str) -> bool {
    engine.pending_actions().iter().any(|a| {
        matches!(a, duroxide::Action::CallActivity { name: n, .. } if n == name)
    })
}
...
pub fn has_timer_action(engine: &ReplayEngine) -> bool {
    engine.pending_actions().iter().any(|a| {
        matches!(a, duroxide::Action::CreateTimer { .. })
    })
}
...
pub fn has_external_action(engine: &ReplayEngine, name: &str) -> bool {
    engine.pending_actions().iter().any(|a| {
        matches!(a, duroxide::Action::WaitExternal { name: n, .. } if n == name)
    })
}
...
pub fn has_sub_orch_action(engine: &ReplayEngine, name: &str) -> bool {
    engine.pending_actions().iter().any(|a| {
        matches!(a, duroxide::Action::StartSubOrchestration { name: n, .. } if n == name)
    })
}
...
pub fn has_activity_scheduled_delta(engine: &ReplayEngine, name: &str) -> bool {
    engine.history_delta().iter().any(|e| {
        matches!(&e.kind, EventKind::ActivityScheduled { name: n, .. } if n == name)
    })
}
...
pub fn has_timer_created_delta(engine: &ReplayEngine) -> bool {
    engine.history_delta().iter().any(|e| {
        matches!(&e.kind, EventKind::TimerCreated { .. })
    })
}
