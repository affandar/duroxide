...
use rust_dtf::{Action, DurableOutput, Event, OrchestrationContext, run_turn};
use std::sync::Arc;
use std::sync::Arc as StdArc;

async fn orchestration_completes_and_replays_deterministically_with(
    ...
    let handle = rt
        .clone()
        .spawn_instance_to_completion("inst-orch-1", orchestration)
        .await;
    ...
    assert_eq!(
        final_history.len(),
        8,
        "expected 8 history events (scheduled + completed)"
    );
...
async fn sequential_activity_chain_completes_with(store: StdArc<dyn HistoryStore>) {
    ...
    let registry = ActivityRegistry::builder()
        .register("A", |input: String| async move {
            input.parse::<i32>().map(|x| x + 1).unwrap_or(0).to_string()
        })
    ...
    let handle = rt
        .clone()
        .spawn_instance_to_completion("inst-seq-1", orchestrator)
        .await;
    let (final_history, output) = handle.await.unwrap();
    ...
    assert_eq!(
        final_history.len(),
        6,
        "expected exactly three scheduled+completed activity pairs in history"
    );
    rt.shutdown().await;
}
