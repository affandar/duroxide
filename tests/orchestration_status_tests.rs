...
async fn test_status_completed() {
    ...
    let orchestration = |ctx: OrchestrationContext, input: String| async move {
        ctx.schedule_activity("ReturnValue", input)
            .into_activity()
            .await
    };
...
async fn test_status_after_continue_as_new() {
    ...
    assert!(
        history.iter().any(|e| matches!(e, duroxide::Event::OrchestrationStarted { input, .. } if input == "2")),
...
async fn test_status_cancelled() {
    ...
    let _ = client.cancel_instance("test-cancelled", "test requested cancellation").await;
...
async fn test_status_independence() {
    ...
    client
        ...
        .unwrap();
    client
        .start_orchestration("inst-fail", "FailOrch", "")
        .await
        .unwrap();
