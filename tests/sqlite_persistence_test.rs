...
async fn test_sqlite_lock_expiry() {
    ...
    // Phase 1: Create store, dequeue item (creating lock), then drop store
    {
        ...
        let (item, token) = store.dequeue_worker_peek_lock().await.expect("Should have work item");
    ...
    // Phase 3: Wait for lock to expire
    {
        ...
        // Direct SQL query to see the locked item
        {
                ...
                .await
            {
                ...
                println!(
                    "Worker item - id: {}, lock_token: {:?}, locked_until: {:?}",
                    id, lock_token, locked_until
                );
...
async fn test_sqlite_database_file_creation() {
    ...
    println!(
        "Final DB file size: {} bytes (grew by {} bytes)",
        final_size,
