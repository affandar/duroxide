...
- Rename the `ManagementCapability` trait to `ProviderAdmin`.
- Move `list_instances` and `list_executions` from `Provider` into `ProviderAdmin`.
...
- Keep execution ID lookup only in `ProviderAdmin` for management scenarios.
...
- Implement `Provider`, `ProviderAdmin`, `ProviderValidator`, and `ProviderTester` for `SqliteProvider`.
...
- Verify public re-exports expose `Provider`, `ProviderAdmin`, and `ProviderValidator` appropriately.
